...
const _makeHash = Effect.gen(function* () {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;

	const baseCache = new Map<string, Hash.Hash>();
	const extendedCache = new WeakMap<Workspace, Map<WorkspacePath, Hash.Hash>>();
	...
	return Effect.fn("hash")(function* (
		workspace: Workspace,
		file: WorkspacePath,
		scope: Scope,
	) {
