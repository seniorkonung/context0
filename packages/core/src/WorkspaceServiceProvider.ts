...
const _makeDiscover = Effect.gen(function* () {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	return Effect.fn("discover")(function* ($1: AbsolutePath | undefined) {
			...
			Effect.andThen(
				Effect.forEach(
					Effect.fnUntraced(function* (dir) {
						const dirPath = AbsolutePath.makeUnsafe(path.resolve(rootDir, dir));
						const configPath = path.resolve(dirPath, CONTEXT0_CONFIG_FILE_NAME);
						return {
							dir: dirPath,
...
const _makeInit = Effect.gen(function* () {
	const fs = yield* FileSystem.FileSystem;
	const path = yield* Path.Path;
	const discover = yield* _makeDiscover;
	return Effect.fn("init")(function* ($1: AbsolutePath | undefined) {
